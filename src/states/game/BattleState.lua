...
function BattleState:init(player)
    ...
    local opPokemon = self.opponent.party.pokemon[1]
    if opPokemon.name == 'Aardart' then
        table.insert(POKEMON[1], opPokemon)
    elseif opPokemon.name == 'Agnite' then
        table.insert(POKEMON[2], opPokemon)
    elseif opPokemon.name == 'Anoleaf' then
        table.insert(POKEMON[3], opPokemon)
    elseif opPokemon.name == 'Bamboon' then
        table.insert(POKEMON[4], opPokemon)
    elseif opPokemon.name == 'Cardiwing' then
        table.insert(POKEMON[5], opPokemon)
    end

    self.playerSprite = BattleSprite(self.player.party.pokemon[CURRENTLY_SELECTED].battleSpriteBack, 
    ...
    self.playerHealthBar = ProgressBar {
        ...
        value = self.player.party.pokemon[CURRENTLY_SELECTED].currentHP,
        max = self.player.party.pokemon[CURRENTLY_SELECTED].HP
    }
    ...
    self.playerExpBar = ProgressBar {
        ...
        value = self.player.party.pokemon[CURRENTLY_SELECTED].currentExp,
        max = self.player.party.pokemon[CURRENTLY_SELECTED].expToLevel
    }
    ...
    self.playerPokemon = self.player.party.pokemon[CURRENTLY_SELECTED]
...
function BattleState:triggerStartingDialogue()
    ...
    function()
        gStateStack:push(BattleMessageState('Go, ' .. tostring(self.player.party.pokemon[CURRENTLY_SELECTED].name .. '!'),
